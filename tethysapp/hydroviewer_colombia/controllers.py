...
# Forecast plot
def get_forecast_plot(comid, stats, rperiods, low_vals, records):
    ...
    if np.nanmin(corrected_stats_df) < np.nanmin(low_vals.values):

        visible = True

        # Calc threshold value
        value_low_level = np.nanmin(low_vals.values)
        ...
        # Add area to graph        
        hydroviewer_figure.add_trace(template(name = f'Umbral mínimo: {round(value_low_level, 1):.2f}',
                                              y = (min_forecast_data, min_forecast_data, 
                                                   value_low_level, value_low_level),
                                              color = 'black',
...
def _rperiod_scatters(startdate: str, enddate: str, rperiods: pd.DataFrame, 
                      y_max: float, max_visible: float = 0, visible: bool = None):
...
def _lowlevels_scatter(startdate: str, enddate: str, low_vals: pd.DataFrame, 
                       y_max: float, max_visible: float = 0, visible: bool = None):
    ...
    low_val = low_vals['7q10'].values[0]
    low_val_min = 0

    return [template(f'Umbral mínimo: {round(low_val, 1):.2f}', (low_val_min, low_val_min, low_val, low_val), 'black')]
...
def historic_simulation(hist: pd.DataFrame, rperiods: pd.DataFrame = None, 
                        low_vals: pd.DataFrame = None,  titles: dict = False,
    ...
    if rperiods is not None:
        ...
        rperiod_scatters = _rperiod_scatters(startdate, enddate, 
                                             rperiods, plot_data['y_max'], 
                                             plot_data['y_max'])
    else:
        rperiod_scatters = []
    ...
    if low_vals is not None:
        lowlevels_scatters = _lowlevels_scatter(startdate, enddate, 
                                                low_vals, plot_data['y_max'], 
                                                plot_data['y_max'])
    else:
        lowlevels_scatters = []
    ...
    scatter_plots += lowlevels_scatters
...
def get_data(request):
    ...
    try:
        ...
        low_vals = get_warning_low_level(station_comid, simulated_data)
